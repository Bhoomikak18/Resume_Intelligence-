        ...
        with st.spinner("Our AI Agents are collaborating on your profile..."):
            ...
            try:
                ...
                if "invalid_api_key" in error_text.lower() or "401" in error_text:
                    st.error("Groq API key is invalid. Update GROQ_API_KEY in your .env file and rerun.")
                elif "GOOGLE_API_KEY" in error_text or "GROQ_API_KEY" in error_text:
                    st.error(error_text)
                else:
                    st.error("AI analysis failed. Check your API keys and internet connection, then try again.")
                    st.exception(e)
                st.stop()
...
# 5. Footer
st.markdown("---")
st.caption("Powered by Gemini 2.5 Flash Lite & Llama 3.3 via Groq | Built by Deeksha D Shenoy & Bhoomika K")