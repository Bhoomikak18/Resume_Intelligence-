...
def _get_required_key(name: str) -> str:
    value = os.getenv(name, "").strip()
    if not value or "your_" in value.lower():
        raise ValueError(f"Missing or placeholder value for {name}. Please set it in your .env file.")
    return value


def _get_llms():
    # Lazy initialize so app can render even if keys are missing/invalid.
    groq_key = _get_required_key("GROQ_API_KEY")
    google_key = _get_required_key("GOOGLE_API_KEY")

    groq_llm = ChatGroq(model_name="llama-3.3-70b-versatile", groq_api_key=groq_key)
    gemini_llm = ChatGoogleGenerativeAI(model="gemini-2.5-flash-lite", google_api_key=google_key)
